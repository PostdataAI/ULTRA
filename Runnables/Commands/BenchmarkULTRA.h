...
class TraceQueryFC : public ParameterizedCommand {
public:
    TraceQueryFC(BasicShell& shell) :
        ParameterizedCommand(shell, "traceQueryFC", "Trace a single FC query in detail") {
        addParameter("Intermediate input file");
        addParameter("Core CH input file");
    ...
    virtual void execute() noexcept {
        ...
        Vertex source = Vertex(getParameter<int>("Source vertex"));
        Vertex target = Vertex(getParameter<int>("Target vertex"));
        int departureTime = getParameter<int>("Departure time");

        std::cout << "\n=== Tracing Query ===" << std::endl;
        std::cout << "Source: " << source << std::endl;
        std::cout << "Target: " << target << std::endl;
        std::cout << "Departure: " << departureTime << std::endl;

        // Run Classic
        using TDDijkstraClassic = TimeDependentDijkstraStatefulClassic<TimeDependentGraphClassic, TDD::NoProfiler, false, true>;
        TDDijkstraClassic algorithmClassic(graphClassic, intermediateData.numberOfStops(), &ch);

        algorithmClassic.run(source, departureTime, target);
        int classicResult = algorithmClassic.getArrivalTime(target);

        std::cout << "\nClassic result: " << classicResult << "s" << std::endl;

        // Get Classic path
        auto classicPath = algorithmClassic.getPath(target);
        std::cout << "Classic path (" << classicPath.size() << " hops):" << std::endl;
        for (size_t i = 0; i < std::min(classicPath.size(), size_t(10)); ++i) {
            std::cout << "  " << i << ": vertex=" << classicPath[i].vertex
                      << ", arrival=" << classicPath[i].arrivalTime << std::endl;
        }
        if (classicPath.size() > 10) {
            std::cout << "  ... (" << (classicPath.size() - 10) << " more)" << std::endl;
        }

        // Run FC
        using TDDijkstraFC = TimeDependentDijkstraStatefulFC<TDD::NoProfiler, true, true>;  // DEBUG=true
        TDDijkstraFC algorithmFC(graphFC, intermediateData.numberOfStops(), &ch);

        algorithmFC.run(source, departureTime, target);
        int fcResult = algorithmFC.getArrivalTime(target);

        std::cout << "\nFC result: " << fcResult << "s" << std::endl;

        // Get FC path
        auto fcPath = algorithmFC.getPath(target);
        std::cout << "FC path (" << fcPath.size() << " hops):" << std::endl;
        for (size_t i = 0; i < std::min(fcPath.size(), size_t(10)); ++i) {
            std::cout << "  " << i << ": vertex=" << fcPath[i].vertex
                      << ", arrival=" << fcPath[i].arrivalTime << std::endl;
        }
        if (fcPath.size() > 10) {
            std::cout << "  ... (" << (fcPath.size() - 10) << " more)" << std::endl;
        }

        // Compare
        std::cout << "\n=== Comparison ===" << std::endl;
        if (classicResult == fcResult) {
            std::cout << "✓ Results MATCH: " << classicResult << "s" << std::endl;
        } else {
            std::cout << "✗ Results MISMATCH:" << std::endl;
            std::cout << "  Classic: " << classicResult << "s" << std::endl;
            std::cout << "  FC:      " << fcResult << "s" << std::endl;
            std::cout << "  Diff:    " << (fcResult - classicResult) << "s" << std::endl;

            // Find where paths diverge
            std::cout << "\nPath divergence:" << std::endl;
            size_t minLen = std::min(classicPath.size(), fcPath.size());
            for (size_t i = 0; i < minLen; ++i) {
                if (classicPath[i].vertex != fcPath[i].vertex ||
                    classicPath[i].arrivalTime != fcPath[i].arrivalTime) {
                    std::cout << "  First difference at hop " << i << ":" << std::endl;
                    std::cout << "    Classic: vertex=" << classicPath[i].vertex
                              << ", arrival=" << classicPath[i].arrivalTime << std::endl;
                    std::cout << "    FC:      vertex=" << fcPath[i].vertex
                              << ", arrival=" << fcPath[i].arrivalTime << std::endl;

                    // If FC has a suspiciously small arrival time, investigate
                    if (fcPath[i].arrivalTime < 10000) {
                        std::cout << "\n  ⚠️  FC arrival time " << fcPath[i].arrivalTime
                                  << "s is suspiciously small!" << std::endl;
                        std::cout << "      This might be index " << fcPath[i].arrivalTime
                                  << " being used as arrival time." << std::endl;

                        // Check if this is actually an index
                        if (i > 0) {
                            Vertex prevVertex = fcPath[i-1].vertex;
                            Vertex curVertex = fcPath[i].vertex;
                            int prevTime = fcPath[i-1].arrivalTime;

                            std::cout << "\n  Checking edge " << prevVertex << " -> " << curVertex
                                      << " at time " << prevTime << ":" << std::endl;

                            // Check what the edge actually returns
                            for (const Edge e : graphFC.edgesFrom(prevVertex)) {
                                if (graphFC.get(ToVertex, e) == curVertex) {
                                    int arrivalTime = graphFC.getArrivalTime(e, prevTime);
                                    std::cout << "    Edge getArrivalTime(" << prevTime << ") = "
                                              << arrivalTime << std::endl;

                                    const EdgeTripsHandle& h = graphFC.get(Function, e);
                                    std::cout << "    Edge has " << h.tripCount << " trips" << std::endl;

                                    if (h.tripCount > 0) {
                                        const DiscreteTrip* trips = graphFC.getTripsBegin(h);
                                        std::cout << "    First 3 trips:" << std::endl;
                                        for (size_t j = 0; j < std::min((uint32_t)3, h.tripCount); ++j) {
                                            std::cout << "      [" << j << "] dep=" << trips[j].departureTime
                                                      << ", arr=" << trips[j].arrivalTime << std::endl;
                                        }

                                        // Check if FC value matches any index
                                        int suspectValue = fcPath[i].arrivalTime;
                                        if (suspectValue < (int)h.tripCount) {
                                            std::cout << "    ⚠️  Trip at index " << suspectValue << ": dep="
                                                      << trips[suspectValue].departureTime << ", arr="
                                                      << trips[suspectValue].arrivalTime << std::endl;
                                        }
                                    }
                                    break;
                                }
...
class DebugFCStructure : public ParameterizedCommand {
public:
    DebugFCStructure(BasicShell& shell) :
        ParameterizedCommand(shell, "debugFCStructure", "Debug FC structure for a specific vertex") {
        addParameter("Intermediate input file");
        addParameter("Vertex ID");
        addParameter("Departure time");
    }
    ...
    virtual void execute() noexcept {
        ...
        Vertex u = Vertex(getParameter<int>("Vertex ID"));
        int departureTime = getParameter<int>("Departure time");

        std::cout << "\n=== Debugging FC Structure for Vertex " << u << " ===" << std::endl;
        std::cout << "Query departure time: " << departureTime << std::endl;

        const FractionalCascadingData& fc = graphFC.getFCData(u);

        if (fc.m_arr.empty()) {
            std::cout << "No FC data for vertex " << u << std::endl;

            // Find a vertex with FC data
            std::cout << "\nSearching for a vertex with FC data..." << std::endl;
            for (size_t v = 0; v < 1000; ++v) {
                if (graphFC.hasFCData(Vertex(v))) {
                    std::cout << "Found vertex " << v << " with FC data. Using that instead.\n" << std::endl;
                    u = Vertex(v);
                    break;
                }
            }

            const FractionalCascadingData& fc2 = graphFC.getFCData(u);
            if (fc2.m_arr.empty()) {
                std::cout << "Could not find any vertex with FC data in range [0, 1000)" << std::endl;
                return;
            }
        }

        const FractionalCascadingData& fcData = graphFC.getFCData(u);

        std::cout << "\n=== FC Structure for Vertex " << u << " ===" << std::endl;
        std::cout << "FC Structure:" << std::endl;
        std::cout << "  Levels: " << fcData.m_arr.size() << std::endl;
        std::cout << "  Reachable nodes: " << fcData.reachableNodes.size() << std::endl;
        std::cout << "  Walking nodes: " << fcData.walkingNodes.size() << std::endl;

        // Show original array arr[0]
        if (!fcData.arr.empty() && !fcData.arr[0].empty()) {
            std::cout << "\nOriginal departure array (arr[0]):" << std::endl;
            for (size_t i = 0; i < std::min(fcData.arr[0].size(), size_t(10)); ++i) {
                std::cout << "  [" << i << "] = " << fcData.arr[0][i] << std::endl;
            }
            if (fcData.arr[0].size() > 10) {
                std::cout << "  ... (" << (fcData.arr[0].size() - 10) << " more)" << std::endl;
            }
        }

        // Show first level
        std::cout << "\nFirst cascaded array (m_arr[0]):" << std::endl;
        for (size_t i = 0; i < std::min(fcData.m_arr[0].size(), size_t(10)); ++i) {
            std::cout << "  [" << i << "] = " << fcData.m_arr[0][i] << std::endl;
        }
        if (fcData.m_arr[0].size() > 10) {
            std::cout << "  ... (" << (fcData.m_arr[0].size() - 10) << " more)" << std::endl;
        }

        // Perform the lookup
        int loc = fcData.bisectLeft(departureTime);
        std::cout << "\nbisectLeft(" << departureTime << ") in m_arr[0] = " << loc << std::endl;
        if (loc < (int)fcData.m_arr[0].size()) {
            std::cout << "  m_arr[0][" << loc << "] = " << fcData.m_arr[0][loc] << std::endl;
        }

        // Show what the pointer says
        if (!fcData.reachableNodes.empty() && loc < (int)fcData.pointers[0].size()) {
            const FCPointer& ptr = fcData.pointers[0][loc];
            const Vertex targetNode = fcData.reachableNodes[0];

            std::cout << "\nFirst reachable node: " << targetNode << std::endl;
            std::cout << "Pointer at [0][" << loc << "]:  startIndex=" << ptr.startIndex
                      << ", nextLoc=" << ptr.nextLoc << std::endl;

            // Show the actual trips for this edge
            for (const Edge e : graphFC.edgesFrom(u)) {
                if (graphFC.get(ToVertex, e) == targetNode) {
                    const EdgeTripsHandle& h = graphFC.get(Function, e);
                    std::cout << "\nEdge " << u << " -> " << targetNode << ":" << std::endl;
                    std::cout << "  Trip count: " << h.tripCount << std::endl;
                    std::cout << "  Walk time: " << (h.walkTime == never ? -1 : h.walkTime) << std::endl;

                    const DiscreteTrip* trips = graphFC.getTripsBegin(h);
                    std::cout << "\n  First 5 trips:" << std::endl;
                    for (size_t i = 0; i < std::min(h.tripCount, uint32_t(5)); ++i) {
                        std::cout << "    [" << i << "] dep=" << trips[i].departureTime
                                  << ", arr=" << trips[i].arrivalTime << std::endl;
                    }

                    if (ptr.startIndex < (int)h.tripCount) {
                        std::cout << "\n  Trip at startIndex=" << ptr.startIndex << ":" << std::endl;
                        std::cout << "    dep=" << trips[ptr.startIndex].departureTime
                                  << ", arr=" << trips[ptr.startIndex].arrivalTime << std::endl;

                        const int* suffixMin = graphFC.getSuffixMinBegin(h);
                        std::cout << "    suffixMin[" << ptr.startIndex << "] = "
                                  << suffixMin[ptr.startIndex] << std::endl;

                        // What would Classic binary search find?
                        std::cout << "\n  What Classic would find:" << std::endl;
                        auto it = std::lower_bound(trips, trips + h.tripCount, departureTime,
                            [](const DiscreteTrip& trip, int time) { return trip.departureTime < time; });

                        if (it != trips + h.tripCount) {
                            int classicIdx = std::distance(trips, it);
                            std::cout << "    Index: " << classicIdx << std::endl;
                            std::cout << "    dep=" << it->departureTime << ", arr=" << it->arrivalTime << std::endl;
                            std::cout << "    suffixMin[" << classicIdx << "] = " << suffixMin[classicIdx] << std::endl;

                            if (classicIdx != ptr.startIndex) {
                                std::cout << "\n  ❌ MISMATCH: FC startIndex=" << ptr.startIndex
                                          << " vs Classic index=" << classicIdx << std::endl;
                            } else {
                                std::cout << "\n  ✓ FC and Classic agree on the index" << std::endl;
                            }
                        }
                    }
