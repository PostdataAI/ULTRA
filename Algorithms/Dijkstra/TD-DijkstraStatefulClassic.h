...
// SPECIALIZED VERSION FOR TimeDependentGraphClassic with dominated edge filtering.
//
// OPTIMIZATIONS:
...
// 6. Works with filtered connection sets from TimeDependentGraphClassic.
        ...
        while (!Q.empty()) {
            ...
            for (const Edge e : graph.edgesFrom(u)) {
                ...
                if (u < numberOfStops) {
                    ...
                    // CHANGE: Use DiscreteTripClassic* instead of DiscreteTrip*
                    const DiscreteTripClassic* begin = graph.getTripsBegin(atf);
                    const DiscreteTripClassic* end = graph.getTripsEnd(atf);
